 ...
 * or written by the agent (e.g. `workspace/media` →
 * `/sandbox/.openclaw-data/media`). Those look like "escapes" relative to
 * the extraction temp dir on the host, but are intra-sandbox once the
 * backup is restored.
 */
...
export function safeTarExtract(
  ...
  // symlinks baked into the base image or written by the agent.
